...
cp -R ./martitos ${KERNELDIR}/output/$TARGET/
cp ./$TARGET/boot.img ${KERNELDIR}/output/$TARGET/martitos
cp -R ./system ${KERNELDIR}/output/$TARGET/
cp -R ./data ${KERNELDIR}/output/$TARGET/
...
tar -H ustar -c ${KERNELDIR}/output/$TARGET/martitos/boot.img > SM-$TARGET-$AUDIO-kernel-${GETVER}-`date +[%d-%m-%y]`.tar
